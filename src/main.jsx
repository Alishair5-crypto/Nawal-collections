...
import{Search,ShoppingBag,Heart,User,Menu,X,ChevronDown,Plus,Minus,Trash2,ArrowRight,ShieldCheck,Truck,RotateCcw,Settings,Package,Users,Image as ImageIcon,LogOut,CheckCircle,Lock,UserPlus,MessageCircle,MapPin,Phone,Mail,ClipboardList}from"lucide-react";
...
function App(){
 ...
 const[query,setQuery]=useState(""),[filter,setFilter]=useState("All"),[cartOpen,setCartOpen]=useState(false),[admin,setAdmin]=useState(false),[checkout,setCheckout]=useState(false),[menu,setMenu]=useState(false),[editing,setEditing]=useState(null),[selected,setSelected]=useState(null),[authOpen,setAuthOpen]=useState(false),[authMode,setAuthMode]=useState("login"),[pendingOrder,setPendingOrder]=useState(null),[confirmation,setConfirmation]=useState(null),[user,setUser]=useState(null),[profile,setProfile]=useState(null),[loading,setLoading]=useState(true),[notice,setNotice]=useState("");
 ...
 return <div>
  ...
  <header><button className="icon mobile" onClick={()=>setMenu(!menu)}>{menu?<X/>:<Menu/>}</button><div className="logo">ATELIER<span>+</span></div>
   <nav className={menu?"open":""}>{["New In","Summer 26","Lawn 3PC","Luxury","Stitched","Winter","Sale"].map((x,i)=><div className="navItem" key={x}><button onClick={()=>goShop(x==="New In"?"All":x)}>{x}{i>0&&i<6?<ChevronDown size={13}/>:null}</button>{categoryMap[x]&&<div className="mega">{categoryMap[x].map(c=><button key={c} onClick={()=>goChild(x,c)}>{c}</button>)}</div>}</div>)}</nav>
   <div className="actions"><div className="search"><Search size={18}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search products"/></div><button className="icon" onClick={()=>{setAuthMode("login");setAuthOpen(true)}} aria-label="Account"><User/></button><button className="icon" onClick={()=>setNotice(`${wish.length} saved item(s)`)} aria-label="Wishlist"><Heart/><b>{wish.length}</b></button><button className="icon" onClick={()=>setCartOpen(true)} aria-label="Bag"><ShoppingBag/><b>{count}</b></button><button className="adminBtn" onClick={openAdmin} aria-label="Admin"><Settings size={16}/></button></div>
  </header>
  ...
  <main>
   ...
   <section id="shop" className="shop"><div className="sectionHead"><div><p className="eyebrow">SHOP</p><h2>Latest Collection</h2></div><div className="filters">{["All","Lawn 3PC","Shirts","Ready To Wear","Luxury","Winter","Sale"].map(x=><button className={filter===x?"active":""} onClick={()=>goShop(x)} key={x}>{x}</button>)}</div></div>
    <div className="grid">{loading?<p>Loading collection…</p>:shown.length?shown.map(p=><article className="card" key={p.id}><div className="photo" onClick={()=>setSelected(p)}><img src={p.img} alt={p.name}/>{p.badge&&<span>{p.badge}</span>}{p.stock<=0&&<span className="soldBadge">SOLD OUT</span>}<button className="wish" onClick={e=>{e.stopPropagation();setWish(w=>w.includes(p.id)?w.filter(x=>x!==p.id):[...w,p.id])}}>{wish.includes(p.id)?"♥":"♡"}</button><button className="quick" onClick={e=>{e.stopPropagation();add(p)}} disabled={!p.stock}>{p.stock?"ADD TO BAG":"SOLD OUT"}</button></div><div className="meta" onClick={()=>setSelected(p)}><p>{p.cat}</p><h3>{p.name}</h3><strong>{money(p.price)}</strong> {p.old>p.price&&<del>{money(p.old)}</del>}<small>{p.stock>0?`${p.stock} available`:"Sold out"}</small></div></article>):<div className="emptyCollection"><h3>No products found.</h3><p>Publish products from Admin → Products to populate this collection.</p></div>}</div>
   </section>
   ...
   <section className="categories"><p className="eyebrow">EXPLORE</p><h2>Shop by mood</h2><div className="catgrid">{["Lawn 3PC","Ready To Wear","Luxury","Winter"].map(x=><button key={x} onClick={()=>goShop(x)}><img src={categoryHero(x)} alt={x}/><span>{x}<ArrowRight size={16}/></span></button>)}</div></section>
